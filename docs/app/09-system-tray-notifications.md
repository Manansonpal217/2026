...
function buildTrayMenu(session: TraySessionInfo | null): Menu {
  return Menu.buildFromTemplate([
    session
      ? {
          label: `🔴 ${session.taskName} — ${formatDuration(session.elapsedSeconds)}`,
          enabled: false,
        }
      : { label: 'Not tracking', enabled: false },
    { type: 'separator' },
    {
      label: session ? 'Stop & Log' : 'Start Tracking',
      click: () => BrowserWindow.getAllWindows()[0]?.webContents.send('tray:toggle_tracking'),
    },
    {
      label: 'Add Manual Time',
      click: () => BrowserWindow.getAllWindows()[0]?.webContents.send('shortcut:manual_entry'),
    },
    { type: 'separator' },
