...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      try {
        const [usersRes, timeRes, screenshotsRes, approvalsRes, streaksRes] =
          await Promise.allSettled([
            fetch(`${API_URL}/v1/admin/users?status=active&limit=1`, {
              headers: { Authorization: `Bearer ${token}` },
            }),
            fetch(
              `${API_URL}/v1/reports/time?from=${weekStart.toISOString()}&to=${today.toISOString()}&group_by=day&limit=200`,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            ),
            fetch(
              `${API_URL}/v1/screenshots?from=${today.toISOString().split('T')[0]}T00:00:00.000Z&to=${today.toISOString().split('T')[0]}T23:59:59.000Z&limit=1`,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            ),
            fetch(`${API_URL}/v1/sessions/pending-approval?limit=1`, {
              headers: { Authorization: `Bearer ${token}` },
            }),
            fetch(`${API_URL}/v1/admin/streaks`, { headers: { Authorization: `Bearer ${token}` } }),
          ])
        ...
        if (streaksRes.status === 'fulfilled' && streaksRes.value.ok) {
          const d = await streaksRes.value.json()
          setTopStreaks((d.users ?? []).slice(0, 5))
        }
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          return (
            <Link key={label} href={href} className="block group">
              <div className="p-4 rounded-xl border border-border/50 bg-surface/50 hover:bg-surface/80 transition-colors">
                ...
                <p
                  className={`text-2xl font-bold ${loading ? 'text-muted-foreground' : 'text-foreground'}`}
                >
                  {value}
                </p>
      ...
      {/* Time chart + Top Streaks */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 p-4 rounded-xl border border-border/50 bg-surface/50">
        ...
        <div className="p-4 rounded-xl border border-border/50 bg-surface/50">
          <div className="flex items-center justify-between mb-4">
            <h2
              ...
              title="Streak: Consecutive days the user opened TrackSync and tracked time. Each day with at least one completed session counts. If they tracked today, today is included; otherwise it counts backward from their last active day."
            >
              Top Streaks
            </h2>
            <Link href="/admin/team" className="text-xs text-primary hover:underline">
              View team →
            </Link>
          </div>
          {loading ? (
              ...
              {topStreaks.map((u) => (
                <li key={u.id} className="flex items-center justify-between gap-2 py-1.5">
                  ...
                  <span
                    ...
                    title={`${u.streak} day streak — consecutive days with tracked time`}
                  >
                    <span>🔥</span>
                    <span className="text-sm font-medium tabular-nums">{u.streak}</span>
                  </span>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-xs text-muted-foreground py-4">No streak data yet</p>
          )}
        </div>
      </div>
