...
// ── Inline syncPendingSessions logic for testing ──────────────────────────────
...
async function syncPendingSessions(
  db: Database.Database,
  fetchFn: (url: string, init: RequestInit) => Promise<{ ok: boolean; status: number; json: () => Promise<BatchResponse> }>,
  ...
  const unsynced = db
    ...
    .all(BATCH_SIZE) as Array<{ id: string; device_id: string; device_name: string }>
  ...
  if (!res.ok) {
    ...
    for (const s of unsynced) stmt.run(`HTTP ${res.status}`, s.id)
...
describe('syncPendingSessions', () => {
  it('sends unsynced sessions in a batch and marks them synced', async () => {
    ...
    const rows = testDb.prepare('SELECT synced FROM local_sessions').all() as Array<{ synced: number }>
    expect(rows.every((r) => r.synced === 1)).toBe(true)
  })

  it('skips sessions with 6+ failed attempts', async () => {
    insertSession(testDb, { sync_attempts: 6 })
  ...
  it('increments sync_attempts on HTTP error', async () => {
  ...
  it('handles partial server errors — syncs some, fails others', async () => {
    ...
    const r1 = testDb.prepare('SELECT synced FROM local_sessions WHERE id = ?').get(id1) as { synced: number }
    const r2 = testDb.prepare('SELECT sync_attempts, last_sync_error FROM local_sessions WHERE id = ?').get(id2) as { sync_attempts: number; last_sync_error: string }
