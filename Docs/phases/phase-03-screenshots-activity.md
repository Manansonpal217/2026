...
- [ ] `captureAndStore(sessionId)`:
  ...
  export async function captureAndStore(sessionId: string): Promise<void> {
    ...
    getDb().prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, 0, ?)
    `).run(fileId, sessionId, filePath, new Date().toISOString(), payload.length, new Date().toISOString())
    // 6. Notify user
...
- [ ] Start/stop with timer:
  ...
  export function startInputMonitoring(): void {
    ioHook.on('keydown', () => { keyboardCount++ })
    ioHook.on('mouseclick', () => { mouseClickCount++ })
  ...
  export function flushAndReset(): { keyboard: number; mouseClicks: number; mouseDistance: number } {
    const counts = { keyboard: keyboardCount, mouseClicks: mouseClickCount, mouseDistance: Math.floor(mouseDistancePx) }
    keyboardCount = 0; mouseClickCount = 0; mouseDistancePx = 0
    return counts
  }
  ```
